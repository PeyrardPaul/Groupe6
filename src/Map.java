			...
			for (int j = 0; j < colonne; j++) {
				if (map[i][j] == 'O') {
					...
					if ((i + a) >= 0 && (j + b) >= 0 && (i + a) < 12 && (j + b) < 12) {
						...
						// y est vrai tant que la liste n'est pas parcourue enti�rement
						// z est vrai tant que la position du joueur n'est pas  celle d'un pi�ge
						boolean z = true;
						String nxt;
						// on a pas besoin de la variable y pour tester si le pi�ge n'est pas encore parcourou
						while (it.hasNext() && z) {
